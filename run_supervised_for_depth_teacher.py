...
def _pred_for_loss_and_metrics(raw):
    """Some depth models return a dict {'rgb': ..., 'rgb_depth': tensor}; use fused RGB-D head."""
    if isinstance(raw, dict):
        raw = raw.get('rgb_depth')
        if raw is None:
            raise ValueError("Model returned a dict without 'rgb_depth' (depth path required for this trainer).")
...
class SupervisedTrainer(Trainer):
    """Single-model fully supervised trainer 
        we want to test the performance of the model trained with both rgb and depth images
    """
    ...
    def get_Trainer_ckpt(self) -> dict:
        """Checkpoint for SmartSaveHook: single model state (rgb and depth encoder)."""
        return {'model': self.model.state_dict()}


class SupervisedEvalHook(EvalHook):
    """Eval hook for a single model (rgb and depth encoder)."""
...
def training(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    model_name = cfg.get('model.name', 'Depth_W_ACM_ResNet34U_f_EMAEncoderOnly')
    ...
    hook_builder(SmartSaveHook,
                 ...
                 save_name=cfg.get('Hook.SmartSaveHook.save_name', 'depth_rgb_train_supervised'),
                 criteria=cfg.get('Hook.SmartSaveHook.criteria', 'test_Dice'))
    hook_builder(LoggerHook, logger_file=cfg.get('Hook.LoggerHook.logger_file', 'logs/depth_rgb_train_supervised.json'))
    hook_builder(MLFlowLoggerHook,
                 ...
                 experiment_name=cfg.get('Hook.MLFlowLoggerHook.experiment_name', 'depth_rgb_train_supervised'),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fully supervised Depth_W_ACM_ResNet34U_f_EMAEncoderOnly with 10% labeled (same style as emaEncoderOnlyTrain).')
