...
from utils.build_dataset_depth_only import build_dataset_depth_only
...
class DepthOnly_MT_Trainer_EMAEncoderOnly(Trainer):
    """
    Mean Teacher trainer for DepthOnlyResNet34U_f_EMAEncoderOnly.
    Trains on raw depth (npy) data only without RGB images.
    Teacher returns a single tensor (no dict). EMA copies student encoder -> teacher encoder only.
    """
    ...
    def run_step_(self) -> None:
        ...
        phase1_info = {'labeled_loss': [], 'unlabeled_depth_loss': [],
                       'consistency_weight': [], 'unlabeled_depth_cutmix_loss': [], 'loss': []}
        phase2_info = {'teacher_labeled_loss': []}

        # ========== PHASE 1: Train Student + EMA (encoder only) ==========
        for batch_id, data in enumerate(self.train_dataloader):
            self.stu_optimizer.zero_grad()
            depth_s, depth, label = data['depth_s'], data['depth'], data['label']
            depth_s, depth, label = depth_s.to(device), depth.to(device), label.to(device)

            unlabeled_depth = depth[self.labeled_bs:]
            unlabeled_depth_s = depth_s[self.labeled_bs:]
            label = label[:self.labeled_bs]

            stu_pred = self.stu_model(depth_s)
            labeled_stu = stu_pred[:self.labeled_bs]
            unlabeled_stu = stu_pred[self.labeled_bs:]

            with torch.no_grad():
                tea_output = self.tea_model(unlabeled_depth)

            # Apply DPA augmentation on depth
            unlabeled_depth_s_cutmix, ema_pred_u_cutmix = dpa(
                unlabeled_depth, unlabeled_depth_s, tea_output, beta=0.3, t=self.current_epoch, T=self.num_epochs
            )
            pred_u_cutmix = self.stu_model(unlabeled_depth_s_cutmix)
            loss_consist_depth_cutmix = self.dpa_loss(pred_u_cutmix, ema_pred_u_cutmix)

            loss_sup = self.class_criterion(labeled_stu, label)
            loss_consist_depth = self.consistency_criterion(unlabeled_stu, tea_output)
            consistency_weight = self._get_current_consistency_weight(
                global_step=batch_id + self.current_epoch * len(self.train_dataloader)
            )
            total_loss = loss_sup + consistency_weight * loss_consist_depth + loss_consist_depth_cutmix
            ...
            phase1_info['unlabeled_depth_loss'].append(loss_consist_depth.item())
            phase1_info['unlabeled_depth_cutmix_loss'].append(loss_consist_depth_cutmix.item())
        ...
        for batch_id, data in enumerate(self.train_dataloader):
            ...
            tea_labeled_output = self.tea_model(labeled_depth)
...
class DepthOnlyEvalHook_EMAEncoderOnly(EvalHook):
    """Eval hook for depth-only training when teacher returns a single tensor (no dict)."""
    ...
    def _run_validation(self) -> dict[typing.Any, typing.Any]:
        ...
        with torch.no_grad():
            for data in self.eval_data_loader:
                ...
                stu_output = self.trainer.stu_model(depth)
                cur_stu_metrics = evaluate(stu_output, gt)
                tea_output = self.trainer.tea_model(depth)
                cur_tea_depth_metrics = evaluate(tea_output, gt)
                for key, value in cur_stu_metrics.items():
                    metrics['stu_' + key].append(value)
                for key, value in cur_tea_depth_metrics.items():
                    metrics['tea_depth_' + key].append(value)
...
def training(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    train_dataloader, val_dataloader, test_dataloader = build_dataset_depth_only(cfg)
    ...
    pretrained = _to_bool(cfg.get('model.pretrained', True))
    print(f"Model ImageNet pretrained: {pretrained}")

    stu_model = getattr(models, cfg.get('model.stu_model.name'))(
        num_classes=cfg.get('model.num_channels_output'),
    ...
    trainer = DepthOnly_MT_Trainer_EMAEncoderOnly(
    ...
    if val_dataloader is not None:
        hook_builder(DepthOnlyEvalHook_EMAEncoderOnly, eval_data_loader=val_dataloader,
                     eval_every_epoch=int(cfg.get('Hook.MeanTeacherEvalHook.eval_every_epoch')), prefix='val_')
    hook_builder(DepthOnlyEvalHook_EMAEncoderOnly, eval_data_loader=test_dataloader,
                 eval_every_epoch=int(cfg.get('Hook.MeanTeacherEvalHook.eval_every_epoch')), prefix='test_')

    hook_builder(LoggerHook, logger_file='logs/depthOnly.json')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Depth-only Mean Teacher with EMA encoder (DepthOnlyResNet34U_f_EMAEncoderOnly).')
    parser.add_argument('--optuna_trial_times', type=int, default=0, help='Optuna trials; 0 = no Optuna.')
    parser.add_argument('--config', type=str, default='cfg/depthOnly.yaml', help='Path to YAML config')
