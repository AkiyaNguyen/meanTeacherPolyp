...
from utils.lora_utils import inject_vector_lora_into_dav2
...
class DAv2Fusion_MT_Trainer_addDepthTrainSignal(Trainer):
    ...
    def _freeze_dav2_backbone(self) -> None:
        """Ensure DAv2 backbone is frozen, EXCEPT for LoRA parameters."""
        if hasattr(self.tea_model, 'dav2_encoder'):
            for name, param in self.tea_model.dav2_encoder.named_parameters():
...
class DAv2DistillTrainer(DAv2Fusion_MT_Trainer_addDepthTrainSignal):
    def __init__(self, *args, distill_weight=0.1, ref_encoder=None, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.distill_weight = float(distill_weight)
        self.ref_encoder = ref_encoder
        if self.ref_encoder is not None:
            self.ref_encoder.eval()
            for p in self.ref_encoder.parameters():
                p.requires_grad_(False)

    def _extract_backbone_feature(self, encoder_out):
        """Convert diverse backbone outputs to a tensor feature map/token map."""
        if isinstance(encoder_out, torch.Tensor):
            return encoder_out
        if isinstance(encoder_out, (list, tuple)) and len(encoder_out) > 0:
            if isinstance(encoder_out[-1], torch.Tensor):
        ...
        if hasattr(encoder_out, 'hidden_states') and encoder_out.hidden_states is not None:
            hs = encoder_out.hidden_states
            if isinstance(hs, (list, tuple)) and len(hs) > 0 and isinstance(hs[-1], torch.Tensor):
                return hs[-1]
        raise TypeError(f"Unsupported encoder output type for distillation: {type(encoder_out)}")

    def _to_global_feature(self, feat: torch.Tensor) -> torch.Tensor:
        """Map backbone output to [B, C] before cosine similarity."""
        if feat.dim() == 3:
            # ViT tokens: [B, T, C]. Drop CLS when possible and mean-pool tokens.
            if feat.size(1) > 1:
                feat = feat[:, 1:, :]
            return feat.mean(dim=1)
        if feat.dim() == 4:
            # CNN map: [B, C, H, W] -> GAP -> [B, C]
            return F.adaptive_avg_pool2d(feat, 1).flatten(1)
        if feat.dim() == 2:
            return feat
        raise ValueError(f"Unsupported feature shape for distillation: {tuple(feat.shape)}")
    ...
    def run_step_(self) -> None:
        ...
        phase1_info = {'labeled_loss': [], 'unlabeled_rgbd_loss': [],
                       'consistency_weight': [], 'unlabeled_rgbd_cutmix_loss': [], 'loss': []}
        phase2_info = {'teacher_labeled_loss': [], 'depth_learn_from_stu_loss': [], 'distill_loss': [], 'loss': []}

        # ========== PHASE 1: keep identical to baseline ==========
        for batch_id, data in enumerate(self.train_dataloader):
            self.stu_optimizer.zero_grad()
            img_s, img, label = data['image_s'], data['image'], data['label']
            img_s, img, label = img_s.to(device), img.to(device), label.to(device)
        ...
        # ========== PHASE 2: teacher + feature distillation ==========
        for _, data in enumerate(self.train_dataloader):
            self.tea_optimizer.zero_grad()
            img, label = data['image'], data['label']
            img, label = img.to(device), label.to(device)
            ...
            # Feature distillation: keep LoRA-tuned backbone close to frozen geometric prior.
            f_lora = self._extract_backbone_feature(
                self.tea_model.dav2_encoder(pixel_values=img, output_hidden_states=True)
            )
            with torch.no_grad():
                f_orig = self._extract_backbone_feature(
                    self.ref_encoder(pixel_values=img, output_hidden_states=True)
                )
            ...
            phase2_info['teacher_labeled_loss'].append(loss_tea_sup.item())
            phase2_info['depth_learn_from_stu_loss'].append(depth_learn_from_stu_loss.item())
            phase2_info['distill_loss'].append(distill_loss.item())
            phase2_info['loss'].append(total_loss.item())
...
def training_distill(cfg: Config, trial: typing.Optional[optuna.trial.Trial] = None):
    ...
    # Snapshot frozen teacher backbone BEFORE LoRA injection for feature distillation target.
    ref_encoder = copy.deepcopy(tea_model.dav2_encoder).to(device)
    ref_encoder.eval()
    for p in ref_encoder.parameters():
        p.requires_grad_(False)

    # Keep the DAv2 backbone frozen before optional LoRA injection.
    if hasattr(tea_model, 'dav2_encoder'):
        for p in tea_model.dav2_encoder.parameters():
            p.requires_grad_(False)

    if cfg.get('lora_params.enable_lora', False):
        if not hasattr(tea_model, 'dav2_encoder'):
            raise AttributeError('Teacher model does not have dav2_encoder for Vector-LoRA injection')
        inject_vector_lora_into_dav2(tea_model.dav2_encoder)
    ...
    # Freeze teacher RGB encoder for phase 2 optimization.
    if hasattr(tea_model, 'rgb_encoder'):
        for p in tea_model.rgb_encoder.parameters():
            p.requires_grad_(False)

    # Dynamically collect trainable teacher params; set LoRA params to 0.1x lr.
    base_lr = float(cfg.get('optimizer.lr'))
    lora_lr = base_lr * 0.1
    tea_lora_params, tea_non_lora_params = [], []
    for name, p in tea_model.named_parameters():
        if not p.requires_grad:
            continue
        if 'lora_' in name:
            tea_lora_params.append(p)
        else:
            tea_non_lora_params.append(p)

    tea_param_groups = []
    if len(tea_non_lora_params) > 0:
        tea_param_groups.append({'params': tea_non_lora_params, 'lr': base_lr})
    if len(tea_lora_params) > 0:
        tea_param_groups.append({'params': tea_lora_params, 'lr': lora_lr})

    tea_optimizer = torch.optim.SGD(tea_param_groups,
    ...
    trainer = DAv2DistillTrainer(
        ...
        distill_weight=float(cfg.get('Trainer.distill_weight', 0.1)),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='cfg/DEMT_DAv2_Distill.yaml', help='Path to YAML config')
    args, unknown = parser.parse_known_args()
    cfg = Config(config_file=args.config, cli_overrides=unknown)

    if args.optuna_trial_times == 0:
        score = training_distill(cfg)
        print(f"Score: {score}")
    else:
        def objective(trial):
            trial_cfg = copy.deepcopy(cfg)
            return training_distill(trial_cfg, trial)
