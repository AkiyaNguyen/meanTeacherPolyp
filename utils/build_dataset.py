...
def _labeled_num_for_split(train_num, cfg):
...
def build_dataset(cfg):
    ...
    if val_perc == 0:
        ...
        labeled_num = _labeled_num_for_split(train_num, cfg)
        print(f"Labelled images: {labeled_num}")
        print(f"Unlabelled images: {train_num - labeled_num}")
        if cfg.get('data.fully_supervised_train', False):
            # Same labeled indices as TwoStreamBatchSampler(..., shuffle=False): first `labeled_num` samples.
            ...
            print("Train loader: fully supervised (Subset + batch_size, no TwoStreamBatchSampler).")
        else:
